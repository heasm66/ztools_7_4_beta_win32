...
#endif
{
	...
	(void) fprintf (
	    stderr,
	    "TXD version 7/4 beta - disassemble Infocom story files. By Mark Howell\n");
	...
	(void) fprintf (
	    stderr, "\t-S n Dump high strings only, starting at address n\n");
	exit (EXIT_FAILURE);
    }
...
#endif
{
	...
	if (i == pcindex) {
		...
		for (; vars; vars--) {
		    tx_printf ("%04x",
			       (unsigned int) read_data_word (&decode.pc));
		    if (vars > 1)
			tx_printf (", ");
		}
		tx_fix_margin (0);
...
#endif
{
	...
	*opers = 1;
	...
	status = decode_parameter (
	    (opcode.opcode & 0x40) ? VARIABLE : BYTE_IMMED, 0);
	if (status)
	    return (status);
	if (decode.first_pass == 0) {
	...
	status = decode_parameter (
	    (opcode.opcode & 0x20) ? VARIABLE : BYTE_IMMED, 1);
	if (status)
	    return (status);
	*opers = 2;
