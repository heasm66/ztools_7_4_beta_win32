...
static void show_words (unsigned int, unsigned long, unsigned int,
			unsigned int);
 ...
 //   }
	...
	*parser_type = infocom_fixed;
	...
	if (((second_entry - first_entry) % 3) == 1) {
		...
		for (i = 0;
		     i < (int) entry_count && (*parser_type == inform_gv1);
		     i++) {
			...
			*parser_type = inform_gv2;
		    ...
		    read_data_byte (
			&first_entry); /* action number.  This can be anything */
		}
	    } else {
		*parser_type = inform_gv2;
	    }
...
#endif
{
	...
	if (parser_type == infocom6_grammar) {
	    ...
	    verb_address = (unsigned int)
		address; /* cast is guaranteed to work provided unsigned int >= 16 bits */
		...
		while (entry_count--) {
			...
			parse_data =
			    (unsigned int) read_data_word (&verb_entry);
			tx_printf ("%04x", (unsigned int) parse_data);
			if (j < (verb_size - 1))
			    tx_printf (" ");
		    }
		...
		if (parser_type < inform_gv2) {
		    for (j = 0; j < verb_size; j++) {
			parse_data =
			    (unsigned int) read_data_byte (&verb_entry);
...
#endif
{
	...
	if (parser_type == infocom6_grammar) {
		...
		while (entry_count--) {
		    ...
		    verb_entry +=
			8; /* skip preposition and direct object and preposition and indirect object*/
		}
	    }
	} else {
...
#endif
{
	...
	if (parser_type == infocom6_grammar) {
		...
		while (entry_count--) {
		    ...
		    if (action_index == (unsigned int) action) {
			vname_address =
			    lookup_word (0, i, VERB_V6, (int) parser_type);
			if (vname_address != 0)
			    decode_text (&vname_address);
			else
			    tx_printf ("'no-verb'");
		...
		if (action_index == (unsigned int) action) {
		    vname_address = lookup_word (0, VERB_NUM (i, parser_type),
						 VERB, (int) parser_type);
		    if (vname_address != 0)
			decode_text (&vname_address);
		    else
			tx_printf ("no-verb");
